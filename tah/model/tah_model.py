    ...
    def forward(
        ...
        **kwargs,
        ...
        while iter_depth < self.max_iter and current_iter_mask.any():
            ...
            # Step1's oracle decider only consumes iter-count labels.  Avoid
            # retaining every layer activation unless the active decider has
            # explicitly configured hidden-state layers (the Step2 MLP path).
            decider = self._active_iter_decider
            needs_hidden_states = hasattr(decider, "hidden_states_layer_nums")
            ...
            all_hidden = self._stack_hidden_states(outputs, device) if needs_hidden_states else None
